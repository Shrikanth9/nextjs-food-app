...
export async function getMeals() {
  if (typeof window === 'undefined') {
    // Server-side: Return the default meals
    return meals;
  }
...
export function getMeal(slug) {
  ...
  try {
    const savedMeals = JSON.parse(localStorage.getItem('savedMeals') || '[]');
    return [...meals, ...savedMeals].find(meal => meal.slug === slug);
  } catch (error) {
...
export async function saveMeal(meal) {
  if (typeof window === 'undefined') {
    // Server-side: Return a dummy response
    return { success: true, message: 'Meal saved successfully (demo mode)' };
  }
