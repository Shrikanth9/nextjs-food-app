// In-memory database with all the dummy meals
const meals = [
  {
    id: 1,
...
export async function getMeals() {
  // Always return default meals on server-side
  if (typeof window === 'undefined') {
    return meals;
  }
  
  // Client-side: Combine default meals with saved meals
  try {
    // Check if localStorage is available
    if (typeof localStorage !== 'undefined') {
      const savedMeals = JSON.parse(localStorage.getItem('savedMeals') || '[]');
      return [...meals, ...savedMeals];
    }
    return meals;
  } catch (error) {
...
export function getMeal(slug) {
  // Server-side: Check only default meals
  if (typeof window === 'undefined') {
    return meals.find(meal => meal.slug === slug);
  }
  
  // Client-side: Check both default and saved meals
  try {
    // Check if localStorage is available
    if (typeof localStorage !== 'undefined') {
      const savedMeals = JSON.parse(localStorage.getItem('savedMeals') || '[]');
      const allMeals = [...meals, ...savedMeals];
...
// Save meal to localStorage (client-side only)
export async function saveMeal(meal) {
  // Server-side or if window is not defined
  if (typeof window === 'undefined' || typeof localStorage === 'undefined') {
    console.warn('saveMeal called in an environment without localStorage');
    return { 
      success: true, 
      message: 'Meal saved in demo mode (not persisted)',
      data: {
        ...meal,
        id: Date.now(),
  ...
  try {
    ...
    return { 
      success: false, 
      error: error.message,
      message: 'Failed to save meal. See console for details.'
    };
  }
}
