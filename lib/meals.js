...
export async function getMeal(slug) {
  try {
    ...
    if (data && data.image) {
      ...
      const cleanImage = data.image.replace(/^\/+|\/+$/g, '');
      return {
        ...data,
        image: `https://tqpmfridizpcasxblnni.supabase.co/storage/v1/object/public/meals/${cleanImage}`
      };
    }
...
export async function saveMeal(meal) {
  try {
    ...
    // Get file extension from the original image
    const fileExtension = meal.image.split('.').pop();
    
    // Create new image name using the slug
    const newImageName = `${slug}.${fileExtension}`;
    const newImagePath = `images/${newImageName}`;

    // If the image is a file object (from file input), we need to handle the upload
    if (meal.image instanceof File) {
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from('your-bucket-name') // Replace with your bucket name
        .upload(newImagePath, meal.image);
